...
import re
HIP_VERSION = 0.0 if torch.version.rocm is None else float(re.search(r"^\d+\.\d+", torch.version.rocm)[0])  #TODO  check if torch.version.hip can be replaced with torch.version.rocm 
